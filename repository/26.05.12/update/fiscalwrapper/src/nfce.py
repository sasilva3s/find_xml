...
import iso8601
from comp_exceptions import FiscalValidation, SafeRebuildRequiredError
...
AUTHORIZED_KEY_IN_REASON = re.compile(r"(?:chNFe|Chave de Acesso)\W*(\d{44})", re.IGNORECASE)
NFE_KEY_INVARIANT_LENGTH = 34
...
def get_authorized_key_from_reason(x_motivo, sent_key):
    # type: (Optional[str], Optional[str]) -> Optional[str]

    match = AUTHORIZED_KEY_IN_REASON.search(x_motivo or "")
    if match is None or not sent_key:
        return None

    authorized_key = match.group(1)
    if authorized_key[:NFE_KEY_INVARIANT_LENGTH] != sent_key[:NFE_KEY_INVARIANT_LENGTH]:
        return None

    return authorized_key


def get_nfe_key_from_xml(nfe_xml):
    # type: (str) -> Optional[str]

    try:
        inf_nfe = remove_xml_namespace(nfe_xml).find(INF_NFE)
    except (Exception,):
        logger.warning("Nao foi possivel ler a chave: XML ilegivel")
        return None

    if inf_nfe is None:
        logger.warning("Nao foi possivel ler a chave: infNFe ausente")
        return None

    return inf_nfe.attrib["Id"][3:]


def extract_nfe_element(xml):
    # type: (str) -> Optional[str]

    try:
        return xml[xml.index("<NFe"):xml.index("</NFe>") + 6]
    except ValueError:
        return None


def nfe_xml_if_key_matches(xml, nfe_key):
    # type: (str, str) -> Optional[str]

    nfe_xml = extract_nfe_element(xml=xml)
    if nfe_xml is None or get_nfe_key_from_xml(nfe_xml=nfe_xml) != nfe_key:
        return None
...
class NfceRequestBuilder:
    ...
    def build_request(self, order, contingencia, dh_contingencia, just_contingencia, force_fresh_emission=False):
        nfe, data_emissao, serie_nota, numero_nota, nf_model = self._build_nfe(
            ...
            force_fresh_emission=force_fresh_emission,
        )
    ...
    def _build_nfe(
        ...
        force_fresh_emission=False,  # type: bool
    ):
        ...
        context = {
            ...
            ContextKeys.force_fresh_emission: force_fresh_emission,
        }
...
class NfceProcessor(FiscalProcessor):
    ...
    def request_fiscal(self, posid, order, tenders, paf=False, fiscal_id=None):
        ...
        if not contingencia and self.nfce_autorizador:
            ...
            except SafeRebuildRequiredError as safe_rebuild_error:
                logger.warning(
                    "Rejection %s on Order %s: safe rebuild (fresh dhEmi) instead of offline contingency",
                    safe_rebuild_error.cstat,
                    order_id,
                )

                if safe_rebuild_error.response_xml is not None:
                    rejection_xml_base64 = base64.b64encode(safe_rebuild_error.response_xml)
                    rejection_log = "Safe rebuild rejection. OrderId: [{}] - cStat: [{}] - ResponseXML: [{}]"
                    loggerFiscalXml.info(rejection_log.format(order_id, safe_rebuild_error.cstat, rejection_xml_base64))

                rebuilt = self._try_safe_rebuild(
                    order=order,
                    order_id=order_id,
                    original_request=request,
                )
                if rebuilt is not None:
                    request, response_consulta, data_emissao, serie_nota, numero_nota, nfe_key = rebuilt
                    request_xml = self._get_xml_request(request=request)
                    dir_arquivo = os.path.join(data_emissao[0:4], data_emissao[5:7], data_emissao[8:10])
                    dir_enviados = os.path.join(self.fiscal_sent_dir, "Enviados", dir_arquivo)
                    if not os.path.exists(dir_enviados):
                    ...
                    logger.warning(
                        "Safe rebuild did not authorize Order %s; entering contingency to avoid losing the note",
                        order_id,
                    )
    ...
    def _try_safe_rebuild(
        self,               # type: NfceProcessor
        order,              # type: eTree.Element
        order_id,           # type: str
        original_request,   # type: str
    ):
        # type: (...) -> Optional[Tuple[str, str, str, str, str, str]]

        rejected_index = original_request.index("Id=\"NFe")
        rejected_key = original_request[rejected_index + 4:rejected_index + 51]
        logger.info("Safe rebuild of Order %s. Rejected key: %s", order_id, rejected_key)

        for attempt in range(self.SAFE_REBUILD_MAX_ATTEMPTS):
            logger.info(
                "Safe rebuild of Order %s: attempt %d of %d with fresh dhEmi",
                order_id,
                attempt + 1,
                self.SAFE_REBUILD_MAX_ATTEMPTS,
            )
            request, data_emissao, serie_nota, numero_nota, nf_model = self.nfce_request_builder.build_request(
                order=order,
                contingencia=False,
            ...
            try:
                ...
                logger.info(
                    "Safe rebuild of Order %s authorized with fresh dhEmi. New key: %s",
                    order_id,
                    nfe_key,
                )
                return request, response_consulta, data_emissao, serie_nota, numero_nota, nfe_key

            except SafeRebuildRequiredError as retry_error:
                logger.warning(
                    "Safe rebuild of Order %s still returned %s; retrying with fresh dhEmi",
                    order_id,
                    retry_error.cstat,
                )
                continue

            except (ConnectionError, Timeout) as connection_error:
                logger.warning(
                    "Safe rebuild of Order %s interrupted by SEFAZ unavailability: %s",
                    order_id,
                    connection_error,
                )
                return None
        ...
        logger.warning(
            "Safe rebuild of Order %s exhausted %d attempts without authorization",
            order_id,
            self.SAFE_REBUILD_MAX_ATTEMPTS,
        )
        return None
...
class NfceAutorizador(object):
    ...
    def _verifica_status_processamento_nfce(response_consulta_xml):
        ...
        elif status_nfce in safe_rebuild_reasons:
            logger.warning("Rejection %s - safe rebuild (fresh dhEmi) instead of contingency", status_nfce)
            raise SafeRebuildRequiredError(cstat=status_nfce, response_xml=response_consulta)
...
class NfceContingencia:
    ...
    def _handle_duplicated_diff_key(
        ...
        sent_key = get_nfe_key_from_xml(nfe_xml=order.xml)
        authorized_key = get_authorized_key_from_reason(x_motivo=motivo, sent_key=sent_key)
        if authorized_key is None:
            authorized_key = get_nfe_key_from_xml(nfe_xml=self._rebuild_nfe(order=order))
            loggerThread.warning(
                "539 da order %s sem chave autorizada valida no xMotivo (chave enviada %s). "
                "Tentando a chave do XML remontado: %s" % (order_id_padded, sent_key, authorized_key)
            )

        if authorized_key is None:
            loggerThread.warning(
                "539 da order %s sem chave para consultar. "
                "Marcando como excecao terminal para intervencao manual." % order_id_padded
            )
        ...
        try:
            loggerThread.info(
                "Consultando SEFAZ com a chave %s para a order %s" % (
                    authorized_key,
                    order_id_padded,
                )
            )
            ...
            if not fiscal_data:
                loggerThread.warning(
                    "539 da order %s sem resposta da SEFAZ para a chave autorizada %s. Sera retentado." % (
                        order_id_padded,
                        authorized_key,
                    )
                )
            ...
            loggerThread.info(
                "Order %s: chave autorizada %s retornou cStat %s" % (order_id_padded, authorized_key, c_stat)
            )
            ...
            authorized_xml = self._find_nfe_xml_with_key(
                order=order,
                nfe_key=authorized_key,
            )
            if authorized_xml is None:
                loggerThread.warning(
                    "539 da order %s: nota autorizada na SEFAZ com a chave %s, mas o POS nao tem o XML "
                    "correspondente. Marcando como excecao terminal para conciliacao manual." % (
                        order_id_padded,
                        authorized_key,
                    )
                )
            ...
            loggerThread.info("Order %s resolvida com a chave autorizada %s" % (order_id_padded, authorized_key))
    ...
    def _find_nfe_xml_with_key(
        ...
        order_id_padded = order.order_id.zfill(9)
        nfe_xml = nfe_xml_if_key_matches(xml=order.xml, nfe_key=nfe_key)
        if nfe_xml is not None:
            loggerThread.info("Order %s: XML da nota %s obtido do XMLRequest da order" % (order_id_padded, nfe_key))
            return nfe_xml

        nfe_xml = nfe_xml_if_key_matches(xml=self._rebuild_nfe(order=order), nfe_key=nfe_key)
        if nfe_xml is not None:
            loggerThread.info("Order %s: XML da nota %s obtido da remontagem" % (order_id_padded, nfe_key))
            return nfe_xml
    ...
    def _rebuild_nfe(
        ...
        try:
            return self._replace_contingency_nfe(
                request_str=order.xml,
                order_xml=self._parse_order_picture(order.order_picture),
            )
        except (Exception,):
            loggerThread.warning(
                "Order %s: nao foi possivel remontar o XML a partir do order picture." % order.order_id.zfill(9)
            )
            return ""
